...
import improbable.natures.player.PlayerNature
  ...
  private def engineConnected(msg: EngineConnected): Unit = {
    msg match {
      ...
      case EngineConnected(userId, UNITY_CLIENT, _) =>
        addEntity(userId)
      case EngineConnected(_, UNITY_FSIM, _) =>
        logger.info("Fsim connected.")
      case _ =>
    }
  }
...
object PlayerLifeCycleManager {
  ...
  private val UNITY_FSIM = "UnityFSim"
  var userIdToEntityIdMap = Map[EngineId, EntityId]()
}
